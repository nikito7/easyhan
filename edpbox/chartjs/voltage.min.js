...
var ctx1=new Chart(document.getElementById(canvas).getContext('2d'),{type:'line',data:{labels:timeLabel1,datasets:dataset1,},options:{legend:{display:true,},elements:{point:{radius:((dataSet.length>31)?1:2)}},interaction:{mode:'index',axis:'y'},scales:{y:{beginAtZero:false,display:true,title:{display:false,text:((dataSet.length>31)?'Volts':'Volts'),
color:'#191'}}},plugins:{zoom:zoomOptions,title:{display:true,text:title,padding:{top:15,bottom:5}}}}})
...
$.get('vt-0d.csv',{'_':$.now()},function(csvString){var data=Papa.parse(csvString).data;makeChart('chart1',data,'Tensão - Hoje (V)')})
$.get('vt-1d.csv',{'_':$.now()},function(csvString){var data=Papa.parse(csvString).data;makeChart('chart2',data,'Tensão - Ontem (V)')})
$.get('vt-2d.csv',{'_':$.now()},function(csvString){var data=Papa.parse(csvString).data;makeChart('chart3',data,'Tensão - Anteontem (V)')})}});
